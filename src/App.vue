...
<template>
  ...
  <div class="flex gap-3 py0 px-8">
    <div class="formulario">
      ...
      <div class="w-100 margin-y-3 m-2 flex justify-between align-center">
        <label>Nome:</label>
        <input class="flex-1 ms-2 p-2 text-lg rounded border-solid" type="text" v-model="produto.nome" />
      </div>
      <div class="w-100 margin-y-3 m-2 flex justify-between align-center">
        <label>Preço (em reais):</label>
        <input class="flex-1 ms-2 p-2 text-lg rounded border-solid" type="number" step="0.01" v-model="produto.preco" />
      </div>
      <div class="w-100 margin-y-3 m-2 flex justify-between align-center">
        <label for="">Quantidade:</label>
        <input class="flex-1 ms-2 p-2 text-lg rounded border-solid" type="number" v-model="produto.quantidade" />
      </div>
    ...
    categoria.nome }}
      ...
      <button type="button" class="bg-gray-300 text-black rounded p-2 mt-2"
        @click="mostrarResultado = !mostrarResultado">Mostrar</button>
    </div>
  ...
  <div class="altera-titulo">
    ...
    <input class="w-400 p-2 text-lg rounded border-solid" type="text" v-model="titulo" />
  </div>
</template>
